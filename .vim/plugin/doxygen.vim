...
function g:DoxygenAddComment() abort
    try
...
function g:DoxygenWarnings(...) abort
    " Write all buffers
    wall
    ...
    if len(a:000) > 0
        ...
        let l:input_paths = s:CygwinToWindowsPath(expand('%:p:h'))
    endif
    ...
    let l:doxyfile = [
         ...
         \ 'INPUT = ' .. l:input_paths,
         \ 'OUTPUT_DIRECTORY = ' .. s:CygwinToWindowsPath(l:temp_path)
    \ ]

    " Run Doxygen with doxyfile fed to stdin
    let l:warnings = systemlist('doxygen -', l:doxyfile)
    call delete(l:temp_path, 'rf')
...
command -bar Dox call g:DoxygenAddComment()
